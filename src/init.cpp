...
void PrepareShutdown()
{
    ...
    RenameThread("millenniumclubcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "millenniumclubcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DLN and zDLN money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 15858, 25858));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DLN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DLN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, millenniumclubcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DLN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Millenniumclubcoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:15858"));
    ...
    strUsage += HelpMessageOpt("-backupzmillenniumclubcoin=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDLN minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizemillenniumclubcoinamount=<n>     " + strprintf(_("Keep N MILLENIUMCLUBCOIN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 15959, 25959));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Millenniumclubcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(_("")) + "\n" +
           "\n" +
...
{
    RenameThread("millenniumclubcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Millenniumclubcoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Millenniumclubcoin Core is shutting down."));
...
#endif
    // Make sure only a single Millenniumclubcoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Millenniumclubcoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Millenniumclubcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Millenniumclubcoin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZMILLENIUMCLUBCOINMinted();
                        RecalculateZMILLENIUMCLUBCOINSpent();
                    }
                ...
                // Millenniumclubcoin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Millenniumclubcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Millenniumclubcoin Core to complete") << "\n";
        ...
        bool fEnableZMillenniumclubcoinBackups = GetBoolArg("-backupzmillenniumclubcoin", true);
        pwalletMain->setZMillenniumclubcoinAutoBackups(fEnableZMillenniumclubcoinBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeMillenniumclubcoinAmount = 2;
...
//    }
//
//    nAnonymizeMillenniumclubcoinAmount = GetArg("-anonymizemillenniumclubcoinamount", 0);
//    if (nAnonymizeMillenniumclubcoinAmount > 999999) nAnonymizeMillenniumclubcoinAmount = 999999;
//    if (nAnonymizeMillenniumclubcoinAmount < 2) nAnonymizeMillenniumclubcoinAmount = 2;
    ...
    LogPrintf("Anonymize Millenniumclubcoin Amount %d\n", nAnonymizeMillenniumclubcoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1MILLENIUMCLUBCOIN+1000 == (.1MILLENIUMCLUBCOIN+100)*10
       10MILLENIUMCLUBCOIN+10000 == (1MILLENIUMCLUBCOIN+1000)*10
    */
