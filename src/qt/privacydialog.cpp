                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zMCLBpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMCLBSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMCLB->setEnabled(false);
        ui->pushButtonMintzMCLB->setToolTip(tr("zDLN is currently disabled due to maintenance."));

        ui->pushButtonSpendzMCLB->setEnabled(false);
        ui->pushButtonSpendzMCLB->setToolTip(tr("zDLN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMCLBpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMCLB_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
...
void PrivacyDialog::on_pushButtonSpendzMCLB_clicked()
{
...
{
    ZMillenniumclubcoinControlDialog* zMCLBControl = new ZMillenniumclubcoinControlDialog(this);
    zMCLBControl->setModel(walletModel);
    zMCLBControl->exec();
}

void PrivacyDialog::setZMillenniumclubcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMCLBSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMCLB()
{
    ...
    double dAmount = ui->zMCLBpayAmount->text().toDouble();
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzMCLBAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMCLBSyncStatus->setVisible(fShow);
}
