...
#if defined(NDEBUG)
#error "Millenniumclubcoin cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDLN spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zDLN spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zDLN spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
	if (nHeight == 0) {
        nSubsidy = 2300000 * COIN;
	} else if (nHeight < 200001) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 400001 && nHeight >= 200001) {
        nSubsidy = 4.8 * COIN;
	} else if (nHeight < 600001 && nHeight >= 400001) {
        nSubsidy = 4.6 * COIN;
	} else if (nHeight < 800001 && nHeight >= 600001) {
        nSubsidy = 4.4 * COIN;
	} else if (nHeight < 1000001 && nHeight >= 800001) {
        nSubsidy = 4.2 * COIN;
	} else if (nHeight < 1200001 && nHeight >= 1000001) {
        nSubsidy = 4 * COIN;
	} else if (nHeight < 1400001 && nHeight >= 1200001) {
        nSubsidy = 3.8 * COIN;
    } else if (nHeight >= 1600001 && nHeight <= 1400001) {
        nSubsidy = 3.6 * COIN;
    } else if (nHeight >= 1800001 && nHeight <= 1600001) {
        nSubsidy = 3.4 * COIN;
    } else if (nHeight >= 2000001 && nHeight <= 1800001) {
        nSubsidy = 3.2 * COIN;
    } else if (nHeight >= 2200001 && nHeight <= 2000001) {
        nSubsidy = 3 * COIN;
    } else if (nHeight >= 2400001 && nHeight <= 2200001) {
        nSubsidy = 2.8 * COIN;
    } else if (nHeight >= 2600001 && nHeight <= 2400001) {
        nSubsidy = 2.6 * COIN;
    } else if (nHeight >= 2800001 && nHeight <= 2600001) {
        nSubsidy = 2.4 * COIN;
    } else if (nHeight >= 3000001 && nHeight <= 2800001) {
        nSubsidy = 2.2 * COIN;
    } else if (nHeight >= 3200001 && nHeight <= 3000001) {
        nSubsidy = 2 * COIN;
    } else if (nHeight >= 3400001 && nHeight <= 3200001) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight >= 3600001 && nHeight <= 3400001) {
        nSubsidy = 1.6 * COIN;
    } else if (nHeight >= 3800001 && nHeight <= 3600001) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight >= 4000001 && nHeight <= 3800001) {
        nSubsidy = 1.2 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Millenniumclubcoin
...
{
    RenameThread("millenniumclubcoin-scriptch");
...
void RecalculateZMCLBSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMCLB supply
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zDLN with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMCLBSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MILLENIUMCLUBCOIN to zDLN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMCLB spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDLN serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDLN mints and %d zDLN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Millenniumclubcoin: We use certain sporks during IBD, so check to see if they are
